...
class ResourceIndexController extends Controller
{
    ...
    {
        return ! $this->cardholderIsActive($selectedCardHolder) && $this->cardholderHasLinkedCards($selectedCardHolder);
    }
    ...
    private function cardholdersActivityHandoffSignal(CardHolder $selectedCardHolder): string
    {
        return match (true) {
            ...
            $this->cardholderHasLinkedCards($selectedCardHolder) => 'Active holder already carries linked-card context for a useful activity handoff review.',
    ...
    private function cardholdersActivityTimelineHandoffDescription(CardHolder $selectedCardHolder): string
    {
        return match (true) {
            ...
            $this->cardholderHasLinkedCards($selectedCardHolder) => 'Operators should carry active status, linked-card evidence, and branch context in the live workspace before trusting any lifecycle-change or merge follow-up.',
    ...
    {
        return match (true) {
            ! $this->shopIsActive($selectedShop) => 'Keep paused status, recovery ownership gaps, and any visible holder or card coverage together before trusting any reopening, reassignment, or scope-recovery discussion.',
            $this->shopHasAssignedManagers($selectedShop)
                && $this->shopVisibleCardholderCount($selectedShop) > 0
    ...
    {
        return match (true) {
            ! $this->shopIsActive($selectedShop) => 'Paused branch should stay in recovery handoff-only posture until ownership and scope approval are explicit.',
            $this->shopHasAssignedManagers($selectedShop)
                && $this->shopVisibleCardholderCount($selectedShop) > 0
    ...
    private function shopsSelectedShopDependencyStatus(Shop $selectedShop): array
    {
        return [
            ...
            ['label' => 'Manager posture', 'value' => match (true) {
                ! $this->shopIsActive($selectedShop) && $this->shopHasAssignedManagers($selectedShop) => 'Assigned branch managers are visible in this paused Galaxy branch, but reassignment and recovery follow-up should stay blocked until ownership parity is confirmed.',
                ! $this->shopIsActive($selectedShop) => 'No branch manager is assigned yet, which keeps this paused Galaxy branch safer for recovery and ownership-flow parity review before ownership flows are enabled.',
            ...
            ['label' => 'Coverage posture', 'value' => $this->shopIsActive($selectedShop)
