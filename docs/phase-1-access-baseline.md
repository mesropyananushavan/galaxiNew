...
- That dashboard card now groups the live guardrails by workflow family, renders controller-owned display summaries for each family, keeps those families in a controller-owned branch → holder → card → tier → access-shell order, and carries a controller-shaped intro note for the grouped route lane inventory so the access map stays stable and readable as it grows.
