...
- That dashboard card now carries controller-shaped intro notes for the tracked gates, grouped route lanes, and mapped policies, while still grouping the live guardrails by workflow family, rendering controller-owned display summaries for each family, and keeping those families in a controller-owned branch → holder → card → tier → access-shell order so the access map stays stable and readable as it grows.
