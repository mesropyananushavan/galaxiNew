...
- The access baseline now also tracks four first-live Phase 1 admin route trios as explicit guardrail entries: `admin.shops.index` / `store` / `update`, `admin.cardholders.index` / `store` / `update`, `admin.cards.index` / `store` / `update`, and `admin.roles-permissions.index` / `store` / `update`.
