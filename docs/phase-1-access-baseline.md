...
- Each tracked route guardrail now declares its maturity directly in `config/phase-1-access-baseline.php`, so the dashboard access map no longer infers `policy-backed` versus `shared-shell` status or maturity-split counts from guard-string heuristics.
