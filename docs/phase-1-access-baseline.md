...
- That dashboard card now groups the live guardrails by workflow family and renders controller-owned display summaries for each family, so branch, holder, card, tier, and access-shell entry points stay readable as separate Galaxy foundation lanes without rebuilding the lane header in Blade.
