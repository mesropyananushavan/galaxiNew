...
- The access baseline now also tracks five first-live Phase 1 admin route trios as explicit guardrail entries: `admin.shops.index` / `store` / `update`, `admin.cardholders.index` / `store` / `update`, `admin.cards.index` / `store` / `update`, `admin.card-types.index` / `store` / `update`, and `admin.roles-permissions.index` / `store` / `update`.
...
- The card-type routes keep tier-catalog review and first-write entry points behind the same `CardTypePolicy` read/create/update checks already enforced in `routes/admin.php`.
