...
- That dashboard card now groups the live guardrails by workflow family, renders controller-owned display summaries for each family, and keeps those families in a controller-owned branch → holder → card → tier → access-shell order so the access map stays stable as the inventory grows.
