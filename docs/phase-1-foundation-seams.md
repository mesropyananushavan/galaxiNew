...
- runtime note: the dashboard access-baseline card now tracks the first live `roles-permissions` route trio as first-class guardrail entries alongside the broader gate and policy inventories, now with controller-prepared HTTP method and URI contract text from Laravel's router
