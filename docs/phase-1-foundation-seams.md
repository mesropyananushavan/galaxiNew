...
- admin runtime note: `App\Http\Controllers\Admin\DashboardController` now formats seam-source and foundation-seam reference strings before they reach the dashboard surface, and prepares dashboard workspace link paths so review/workspace cards are less URL-assembly-heavy in Blade
...
- current role: keeps the public Galaxy migration doc trail, docs-card heading, metric labels, explanatory notes, config-path callouts, and seam-source bridge references aligned
