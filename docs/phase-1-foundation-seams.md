...
- admin runtime note: `App\Http\Controllers\Admin\DashboardController` now formats seam-source and foundation-seam reference strings before they reach the dashboard surface, prepares dashboard workspace link paths so review/workspace cards are less URL-assembly-heavy in Blade, and shapes the foundation snapshot metric payload before render
