...
- current role: keeps the public Galaxy migration doc trail, docs-card heading, metric labels, explanatory notes, config-path callouts, and seam-source bridge references aligned, with the landing controller now preparing the docs-card heading, summary row payload, and docs inventory payload before render
